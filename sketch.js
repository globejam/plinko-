...
function setup() {
 ...
 HighScoreNumber = database.ref('highscore');
...
// creating boundries
 ...
 var b4 = new Boundary(0, 1011, 50, 562);
  ...
  bounds.push(b5);
  var b6 = new Boundary(0, height/2, 30, height);
  bounds.push(b6);
  var b7 = new Boundary(width, height/2, 40, height);
  bounds.push(b7);
  var b8 = new Boundary(width, 0, height*2, 30);
  bounds.push(b8);
  for (var i = 0; i < cols; i++) {
  ...
  gBall = new GBALL(mouseX,mouseY,16);
}
...
function draw() {
  background("#000000");
gBall.body.position.x=mouseX
gBall.body.position.y=mouseY
  if(score>3000){
    maxParticles=70;
...
//checking if the game fails
  if(createdParticles === maxParticles && frameCount%780===0){
    gamefail();
...
//adds score
 ...
 //shows all text
  ...
  text("Control the yellow ball to push the others", 350, 45);
  textAlign(CENTER);
  textSize(20);
